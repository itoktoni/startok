  ...
  <div>
    ...
    <div class="mb-8">
      ...
      <p class="font-body-sm text-on-surface-variant mt-2">
        Browse and manage work order items, scan barcodes, and track inventory movements.
      </p>
    </div>
    ...
    <section class="mb-6">
      ...
      <div class="flex gap-2 mt-3 overflow-x-auto pb-2">
        <span
          ...
          :key="cat"
          class="inline-flex items-center px-3 py-1 rounded-full font-body-sm text-body-sm cursor-pointer transition-colors"
        ...
        <div class="p-4 flex flex-col gap-3">
          <div class="flex justify-between items-start">
            <div class="flex flex-col">
              <h2 class="font-headline-md text-headline-md text-primary">{{ item.name }}</h2>
              <span class="font-label-caps text-label-caps text-outline">ID: {{ item.id }}</span>
            </div>
          ...
          <div class="flex flex-wrap gap-2">
            <span class="px-2 py-0.5 bg-secondary-container text-on-primary rounded font-label-caps text-label-caps">{{ item.sku }}</span>
            <span class="px-2 py-0.5 bg-tertiary-fixed text-on-tertiary-fixed rounded font-label-caps text-label-caps">{{ item.category }}</span>
          </div>
          <p class="text-on-surface-variant font-body-sm line-clamp-2">{{ item.description }}</p>
          <div class="flex items-center justify-between border-t border-outline-variant pt-3 mt-2">
            <div class="flex flex-col">
              <span class="font-headline-md text-headline-md text-secondary">{{ item.price }}</span>
              <span class="font-label-caps text-label-caps text-outline">{{ item.unit }}</span>
            </div>
            <div class="h-10 w-24 bg-white border border-outline-variant flex items-center justify-center">
              <div class="w-full h-full flex gap-[2px] p-1 items-end">
                <div v-for="(h, i) in item.barcode" :key="i" class="bg-black" :style="{ width: h.w + 'px', height: h.h }"></div>
              </div>
            </div>
          </div>
        </div>
...
const filteredItems = computed(() => {
  return workOrderItems.value.filter((p) => {

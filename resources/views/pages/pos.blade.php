...
function openNote(name,e){e.stopPropagation();noteProd=name;const p=P.find(x=>x.n===name);document.getElementById('noteTitle').textContent=name;document.getElementById('noteInput').value='';const sel=document.getElementById('noteVariant');sel.innerHTML='';const variants=p.v||[];if(variants.length===0){sel.innerHTML='<option value="0">Regular (+Rp 0)</option>';}else{variants.forEach((v,i)=>{const diff=v.p-p.p;const label=diff>=0?'+Rp '+diff.toLocaleString('id-ID'):'-Rp '+Math.abs(diff).toLocaleString('id-ID');sel.innerHTML+=`<option value="${v.p}">${v.n} (${label})</option>`;});}sel.selectedIndex=0;document.getElementById('noteTotal').textContent=fmt(p.p);document.getElementById('noteModal').classList.remove('hidden');}
...
function submitNote(){if(!noteProd)return;const p=P.find(x=>x.n===noteProd);const sel=document.getElementById('noteVariant');const extra=+sel.value;const vLabel=sel.options[sel.selectedIndex].text.split(' (')[0];const note=document.getElementById('noteInput').value;const key=noteProd+'|'+vLabel+'|'+note;const e=cart.find(x=>x.key===key);if(e)e.q++;else cart.push({key,n:p.n,p:p.p,extra:extra,q:1,variant:vLabel,note});rCart();calcT();closeNote();}
